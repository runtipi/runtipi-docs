...
On your first server, navigate to the `runtipi` folder and stop tipi using the CLI
```bash
...
Start Tipi on your second server using the CLI
...
<Callout>
Make sure you have docker installed on your second server before starting Tipi. An easy way to do so is to install a clean tipi instance on your second server, then stop it and replace the runtipi folder with the one you just extracted.
</Callout>
