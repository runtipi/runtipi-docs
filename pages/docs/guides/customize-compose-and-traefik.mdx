...
Now runtipi will not reset the traefik config, you can always change it back to `true` to reset your changes.

### Edit the traefik config

So now that our config is persistent we can safely make our changes by opening the `traefik/traefik.yml` file and changing this:
