...
Open the `runtipi/state/settings.json` file with your preferred editor.
...
Now Runtipi will keep your custom traefik config on start. Change it back to `false` if you want Runtipi to restore the default files on the next restart. So now that our config is persistent we can safely make our changes by opening `traefik/traefik.yml` and changing this:
