...
Create a new script file (`.sh` extension) in the `/path/to/runtipi/scripts` folder (if the scripts folder doesn't exist you can create it manually)
called `backup_runtipi_apps.sh` and paste the following code:
...
```bash
chmod +x backup_runtipi_apps.sh
```
