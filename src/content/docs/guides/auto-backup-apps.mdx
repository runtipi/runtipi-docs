...
runtipiMajorVersion=$(echo $runtipiVersion | cut -d. -f1 | sed 's/^v//')
if [ "$runtipiMajorVersion" -lt 4 ]; then
...
Then we can add our crontab line, in this example we will backup our apps every day of the week at 2 a.m and the first day of the month at 3 a.m.

```bash
# Every day from Tuesday to Sunday at 2 AM - daily
0 2 * * 2-7 /path/to/runtipi/scripts/backup-apps.sh daily

# Every Monday at 2 AM - weekly
0 2 * * 1 /path/to/runtipi/scripts/backup-apps.sh weekly

# Every 1st day of the month at 3 AM - monthly
0 3 1 * * /path/to/runtipi/scripts/backup-apps.sh monthly
```

Make sure that the cron service is enabled and running.
