...
Create the `user-config` folder within `runtipi` and a folder structure matching your app store and apps:
...
```bash
./runtipi-cli app stop jellyfin:<app-store>
./runtipi-cli app start jellyfin:<app-store>
./runtipi-cli app stop pihole:<app-store>
./runtipi-cli app start pihole:<app-store>
```
