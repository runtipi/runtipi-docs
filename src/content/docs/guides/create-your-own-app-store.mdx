...
- **Keep your repository organized** with clear folder structures and naming conventions
...
## Updating your app store

To update your app store, simply push the changes to your repository. The next time Runtipi checks for updates (on restart or when explicitly triggered), it will pull the latest changes.

To manually update your app stores:

```bash
cd /path/to/runtipi
./runtipi-cli app update-store
```
...
- Check that your repository URL is correct in the `settings.json` file
- Ensure your app's `config.json` and `docker-compose.json` files are valid JSON
...
<Callout type="warning">
  If you're updating from an older version of Runtipi to v4.0.0 or newer, make
  sure to follow the [migration
  guide](/docs/reference/breaking-updates#update-from-3xx-to-400-beta) to adapt
  to the new folder structure and dynamic compose requirements.
</Callout>

## Sharing your app store

Once you've created your app store, you can share it with others by providing them with your repository URL. They can add it to their `settings.json` file as described above.
