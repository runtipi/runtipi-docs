...
import { CommandLineIcon, WrenchIcon, ArrowPathIcon } from "@heroicons/react/24/outline";

# Installation

## Introduction

This guide will help you install Runtipi on your server. Make sure your server is secured and you have followed basic security practices before installing Runtipi. (e.g. firewall, ssh keys, root access, etc.) Here is a [good guide](https://www.tecmint.com/initial-ubuntu-server-setup-guide/) to follow.
...
<Callout>
  Not all apps are supported in the arm64 architecture, Runtipi will auto hide
  apps that are not supported by your architecture.
</Callout>
...
<Callout type="warning">
  If you already have Docker installed make sure to have the Docker Compose
  plugin installed. Run `docker compose version` to check if it is installed.
  Instructions to install it can be found
  [here](https://docs.docker.com/compose/install/linux/).
</Callout>

<Callout type="warning">
  If you already have Docker installed make sure you used the official Docker
  install script from [here](https://get.docker.sh) as this guarantees that you
  have the latest build. If you used a command like ```sudo apt install
  docker.io``` you may have an outdated version and issues may occur.
</Callout>
...
```bash
curl -L https://raw.githubusercontent.com/runtipi/runtipi/develop/scripts/install.sh | bash
```
...
<Callout type="warning">
  ...
  as it may lead to breaking your install.
</Callout>
...
<Cards>
  <Cards.Card
    ...
    title="Using the CLI"
    href="/docs/getting-started/using-the-cli"
  />
  <Cards.Card
    icon={<WrenchIcon />}
