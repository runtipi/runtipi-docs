...
| **author**                  | Name of the app maintainer or project                                                                                                                                                                                                                               | Nextcloud                                                                                                                                                                                                                         | yes      |
...
| **supported_architectures** | If the app is only available for specific architectures, you can specify them here. If omitted, both `amd64` and `arm64` are assumed.                                                                                                                               | ["arm64", "amd64"]                                                                                                                                                                                                                | no       |
| **uid**, **gid**            | Optional numeric metadata fields accepted by the schema for app definitions. They are not used by the current backend to change ownership automatically.                                                                                                            | 1000                                                                                                                                                                                                                              | no       |
| **form_fields**             | Form fields let you prompt the user for install-time values such as usernames, passwords, or domains                                                                                                                                                                | See below                                                                                                                                                                                                                         | no       |
| **dynamic_config**          | Marks the app as using Runtipi's dynamic compose format. Prefer `docker-compose.yml` with `x-runtipi`; legacy `docker-compose.json` files are still supported and converted automatically. See [Dynamic Compose](https://runtipi.io/docs/reference/dynamic-compose) | true                                                                                                                                                                                                                              | no       |
...
| **force_pull**              | Always pull the image before install/start/restart instead of relying on the local image cache                                                                                                                                                                      | false                                                                                                                                                                                                                             | no       |
...
| **env_variable**  | The name of the variable you'll reference from the app compose template.                    | NEXTCLOUD_USERNAME                         | ✅       |
...
<Callout>
  When you use the `random` field type for a password or secret, the `min` value
  determines the generated string length. If `min` is omitted, a default length
  of 32 characters is used.
</Callout>
...
<Callout>
  Use `options` only when there are only specific pre-defined values for the
  field. If a user might want to use a custom value, use a normal text field and
  provide common examples in the label.
</Callout>
