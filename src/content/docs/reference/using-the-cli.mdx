...
<Callout type="info">
  Most commands require `sudo` privileges to manage file permissions and Docker
  containers. Running without `sudo` may result in permission errors.
</Callout>
...
Stop and start Runtipi services.
...
Prepare the environment without starting containers. Checks permissions, copies files, and generates configuration.

```bash
sudo ./runtipi-cli prepare [--env-file <path>] [--no-permissions]
```
...
<Callout type="warning">
  Downgrading to previous versions may break your installation. Always backup
  before downgrading.
</Callout>
...
<Callout type="warning">
  This permanently deletes all app data and settings. Backup important data
  first.
</Callout>
...
<Cards>
  ...
  <Cards.Card
    ...
    href="/docs/guides/custom-environment-variables"
  />
  <Cards.Card
